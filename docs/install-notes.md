...
- **mfold-3.6 data path**: `./configure --prefix=` is NOT threaded into the Fortran binaries (`nafold`, `newtemp`, `efn`, `mfold_datdir`), which hardcode `/usr/local/share/mfold/`. `nafold` falls back to CWD so the wrapper symlinks every file from `install/share/mfold/` into each run's tempdir. `newtemp` (invoked whenever `NA=DNA` or `T!=37`) does **not** fall back to CWD — it aborts with `Error opening asint1x2.__ input data file(s)`. Fix: the install script `sed`-patches the hardcoded path in `src/*.f` and `src/efiles.f`, `src/efn.f`, `src/misc-quik.f` to the actual `$PREFIX/share/mfold/` before `./configure && make`. Don't move the install dir after building — the path is baked into the binaries.
